...
class CalibrationPosture(unittest.TestCase):
    ...
    def test_a_grade_is_relaxed_when_the_calibrated_list_no_longer_supports_it(self):
        report = self._report(grade="poor", severity="moderate")
        stage_module._relax_grade(report)
        self.assertEqual(report.condition_grade, "fair")
        self.assertIn("before calibration", report.grade_disagreement)
        self.assertEqual([c.kind for c in report.corrections], ["severity_calibrated"])

    def test_a_grade_is_never_tightened_by_pass_d(self):
        report = self._report(grade="good", severity="major")
        stage_module._relax_grade(report)
        self.assertEqual(report.condition_grade, "good")
        self.assertEqual(report.corrections, [])

    def test_a_truck_with_no_findings_is_not_regraded_here(self):
        report = self._report()
        report.issues = []
        stage_module._relax_grade(report)
        self.assertEqual(report.condition_grade, "poor")
