...
4-minute demo + 3-minute Q&A. The **Appendix** at the end of this file is the long-term research doc
(multi-week product build, US+Türkiye, full observability/pricing stack) — treat it as a reference for
ideas, not the weekend plan. Nothing above the Appendix needs to be read first.
...
narrowing (Appendix §9 scoped to Ford F-MAX only): the brief explicitly says judges bring
...
2. **Evidence (VLM, grounded by prompt structure, not fine-tuned specialists).**
   A weekend has no time to train detectors (Appendix §3's specialist stack is a multi-week
   ...
   ```
   Force every claim to name a `photo_id` — this is the cheap version of the Appendix's
...
3. **Price (a real regression, not vibes).**
   ...
   listings — this alone explains most of the variance (Appendix §9c found R²=0.79 on Turkish
   ...
   satisfies "a range rather than a single number" without needing conformal prediction (Appendix
...
Appendix §9e rather than re-discovering it: TruckMarket (Ford Trucks TR) is already vetted as
...
| Source | Brand(s) | Status (per Appendix §9b/§9e) |
|---|---|---|
...
the Appendix's realized-sale-price problem (§9d.3, needs a dealer agreement) — for a hackathon, asking
...
Everything below is real and in the Appendix, but is multi-week scope, not weekend scope:
...
## Appendix: Long-Term Product Blueprint (Not Weekend Scope)

> This is the long-term/product-scale research doc referenced throughout this plan as "the
> Appendix" (multi-week build, US+Türkiye, full observability and pricing stack). It was
> formerly a separate file, `blueprint.md`; it's merged in here for a single source of truth.
> Ideas here are fair game to borrow into the weekend build above; the roadmap, market sizing
> and US-market content are not part of the hackathon scope — see the Cut List above.
